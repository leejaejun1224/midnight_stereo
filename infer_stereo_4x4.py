...
from modeljj4x4 import StereoModel, assert_multiple
...
def save_color_and_overlay(left_img_path: str,
    ...
    if not (save_color or save_overlay):
        return
    if not HAS_CV2:
        print("[WARN] OpenCV 없음 → overlay 건너뜁니다")
        return
...
# -----------------------------
# ADHOC: 도로 ROI 행 최빈/평균 치환 + bad mask 생성
# -----------------------------
...
def _row_mode_fill_roi(disp: np.ndarray,
    ...
    Returns: (disp_filled, stats_dict, badmask_full)
    """
    H_d, W_d = disp.shape[:2]
    x1, y1, x2, y2 = _map_roi_to_disp(input_size, (W_d, H_d), roi_xyxy_in_input)
...
def _infer_cv_down(h_in: int, w_in: int, h_map: int, w_map: int) -> int:
    """입력 해상도(H,W)와 저해상도 지도(H_map,W_map)로부터 격자 다운비율(cv_down) 추정"""
    if h_map == 0 or w_map == 0:
        return 1
    if h_in % h_map != 0 or w_in % w_map != 0:
        warnings.warn(f"[WARN] 입력({h_in}x{w_in})과 저해상도 지도({h_map}x{w_map})가 나누어떨어지지 않습니다. "
                      f"근사 비율 사용.")
    dh = max(1, int(round(h_in / float(h_map))))
    dw = max(1, int(round(w_in / float(w_map))))
    if dh != dw:
        warnings.warn(f"[WARN] 비정상 격자 비율: dh={dh}, dw={dw}. min(dh,dw) 사용.")
    return min(dh, dw)

def _resize_np(nn: np.ndarray, size_hw: tuple, mode: str = "nearest") -> np.ndarray:
    Ht, Wt = size_hw
    if HAS_CV2:
        interp = cv2.INTER_NEAREST if mode == "nearest" else cv2.INTER_LINEAR
        return cv2.resize(nn, (Wt, Ht), interpolation=interp)
    else:
...
def run_one(model, left_path, right_path, args, device, patch_size, max_disp_px):
    ...
    # --- 저해상도 격자 지도(WTA) ---
    disp_lo_grid = aux["disp_wta"][0, 0].cpu().numpy().astype(np.float32)  # (H_lo, W_lo), 단위: grid step
    H_lo, W_lo = disp_lo_grid.shape

    # cost-volume 격자 다운비율 자동 추정 (1/8 → 8, 1/4 → 4 등)
    cv_down = _infer_cv_down(args.height, args.width, H_lo, W_lo)

    # full-res px 단위로 변환 (저해상도 격자)
    disp_px_lo = (disp_lo_grid * float(cv_down)).astype(np.float32)
...
def main():
    ...
    # 백본 제약(예: ViT/8) 때문에 입력은 여전히 patch_size 배수 권장
