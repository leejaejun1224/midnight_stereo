...
class ResidualConvBlock(nn.Module):
    """
    3x3 (dilation k) -> GN -> GELU -> 3x3 -> GN, Skip
    야간 저대비에서 미세 신호 보존을 위해 잔차 구조 채택
    """
    def __init__(self, ch: int, gn_groups: int = 16, dilation: int = 1):
        super().__init__()
        self.block = nn.Sequential(
            nn.Conv2d(ch, ch, kernel_size=3, stride=1, padding=dilation,
...
class SEBlock(nn.Module):
    """
    채널 어텐션(SE). 야간/저조도에서 약한 신호 채널 강조.
    """
    def __init__(self, ch: int, reduction: int = 8):
        super().__init__()
        hidden = max(ch // reduction, 16)
...
class ConvQuarter(nn.Module):
    """
    얕은 stem → 더 두꺼운 stem + ResidualConvBlock + SE 로 확장.
    in:  [B,3,H,W]
    out: [B,Cc,H/4,W/4]
    """
    def __init__(self, in_ch: int = 3, cc: int = 256, gn_groups: int = 16):
        super().__init__()
        stem_w = 128  # 기존 96 → 128
        self.conv14 = nn.Sequential(
            # H/2
            nn.Conv2d(in_ch, stem_w, 3, 2, 1, bias=False),
            nn.GroupNorm(gn_groups, stem_w),
            nn.GELU(),
            # 동일 해상도에서 한 번 더 추출(야간 미세 패턴 강화)
            nn.Conv2d(stem_w, stem_w, 3, 1, 1, bias=False),
            nn.GroupNorm(gn_groups, stem_w),
            nn.GELU(),
            # H/4
            nn.Conv2d(stem_w, cc, 3, 2, 1, bias=False),
            nn.GroupNorm(gn_groups, cc),
            nn.GELU(),

            # 1/4 해상도에서 용량 증가 + 멀티스케일 수용영역
            ResidualConvBlock(cc, gn_groups=gn_groups, dilation=1),
            ResidualConvBlock(cc, gn_groups=gn_groups, dilation=2),

            # 채널 주의집중(야간 대비 개선)
            SEBlock(cc, reduction=8),
        )
...
class Fusion1_4(nn.Module):
    """
    ViT(업샘플) + Conv(1/4) 융합을 더 깊고 넓게.
    in:  [B, Cc+Cv, H/4, W/4]
    out: [B, Cf,   H/4, W/4]
    """
    def __init__(self, cc: int = 256, cv: int = 384, cf: int = 512, gn_groups: int = 16):
        super().__init__()
        self.fuse = nn.Sequential(
            nn.Conv2d(cc + cv, cf, kernel_size=1, bias=False),
            nn.GroupNorm(gn_groups, cf),
            nn.GELU(),

            # 용량 + 멀티스케일(팽창 1,2)
            ResidualConvBlock(cf, gn_groups=gn_groups, dilation=1),
            ResidualConvBlock(cf, gn_groups=gn_groups, dilation=2),

            # 채널 주의집중
            SEBlock(cf, reduction=8),

            # 마무리 스무딩
            nn.Conv2d(cf, cf, kernel_size=3, stride=1, padding=1, bias=False),
            nn.GroupNorm(gn_groups, cf),
            nn.GELU(),
        )

    def forward(self, c4: torch.Tensor, v8_up: torch.Tensor) -> torch.Tensor:
        # AMP 환경에서 dtype 차이를 자동 정렬 (cat 안전)
        if v8_up.dtype != c4.dtype:
            v8_up = v8_up.to(c4.dtype)
